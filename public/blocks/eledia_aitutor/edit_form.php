 ...
 * generated from {@see \block_eledia_aitutor\local\registry}, and only the keys
 ...
 */
class block_eledia_aitutor_edit_form extends block_edit_form {
    /**
    ...
    protected function specific_definition($mform): void {
        \block_eledia_aitutor\local\formhelper::register_colour_element();
        ...
        if (!empty($this->block->instance->id)) {
            ...
            $actions = \html_writer::link(
                $shelllink,
                get_string('instance_shell_edit_link', 'block_eledia_aitutor'),
        ...
        if ($llmallowed && $grounding) {
            $mform->addElement(
                ...
                [
                    \block_eledia_aitutor\local\chat_mode::MODE_GROUNDED =>
                        get_string('ragmode_grounded', 'block_eledia_aitutor'),
                    \block_eledia_aitutor\local\chat_mode::MODE_LLMONLY =>
                        get_string('ragmode_llmonly', 'block_eledia_aitutor'),
                ]
            );
            $mform->setDefault('config_ragmode', \block_eledia_aitutor\local\chat_mode::MODE_GROUNDED);
    ...
    private function effective_courseid(): int {
        ...
        if ($fixed > 0) {
            return \block_eledia_aitutor\local\security::course_chat_enabled() ? $fixed : 0;
        }

        $passcontext = !isset($config->passcoursecontext) || (int) $config->passcoursecontext === 1;
        $pagecourseid = (int) ($this->block->page->course->id ?? 0);
        if (
            $passcontext && \block_eledia_aitutor\local\security::course_chat_enabled()

 ...
 * users holding block/eledia_aitutor:viewreports while question analytics is
 ...
 * @return void
 */
function block_eledia_aitutor_extend_navigation_course(
    navigation_node $navigation,
    stdClass $course,
    \core\context\course $context
): void {
    if (
        !\block_eledia_aitutor\local\question_log::is_enabled()
            || !has_capability('block/eledia_aitutor:viewreports', $context)
    ) {
        return;
    }

    $navigation->add(
        get_string('report_link', 'block_eledia_aitutor'),
        new moodle_url('/blocks/eledia_aitutor/report.php', ['courseid' => $course->id]),
        navigation_node::TYPE_SETTING,
        null,
        'eledia_aitutorreport',
 ...
 */
function block_eledia_aitutor_pluginfile(
    $course,
    $birecordorcm,
    $context,
    $filearea,
    ...
    $sitefileareas = [
        \block_eledia_aitutor\local\branding::LOGO_FILEAREA,
        \block_eledia_aitutor\local\branding::AVATAR_FILEAREA,
        \block_eledia_aitutor\local\branding::TUTOR_LOGO_FILEAREA,
        \block_eledia_aitutor\local\branding::TUTOR_AVATAR_FILEAREA,
    ];
    $instancefileareas = [
        \block_eledia_aitutor\local\branding::INSTANCE_LOGO_FILEAREA,
        \block_eledia_aitutor\local\branding::INSTANCE_AVATAR_FILEAREA,
    ];
    ...
    $file = $fs->get_file($context->id, 'block_eledia_aitutor', $filearea, $itemid, $filepath, $filename);

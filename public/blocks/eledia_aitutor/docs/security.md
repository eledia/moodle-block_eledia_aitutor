...
- The RAG auth token is stored with `admin_setting_configpasswordunmask`.
- Provisioned tokens are attributed to the component `block_eledia_aitutor`
...
- All external functions `require_login()`, resolve and `validate_context()` a
  caller-supplied context (only block/system contexts accepted), and
  `require_capability()` the appropriate capability.

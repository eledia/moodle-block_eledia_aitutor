...
final class consent_test extends \advanced_testcase {
    ...
    public function test_give_is_documented_and_idempotent(): void {
        ...
        $events = array_filter(
            $sink->get_events(),
            static fn($e) => $e instanceof \block_eledia_aitutor\event\consent_given
        );
    ...
    public function test_require_consent_throws(): void {
        ...
        $this->expectExceptionMessage(get_string('error_consentrequired', 'block_eledia_aitutor'));
    ...
    public function test_user_deleted_observer(): void {
        ...
        \block_eledia_aitutor\local\usage::increment((int) $user->id);
        \block_eledia_aitutor\local\usage::increment((int) $other->id);

        delete_user($user);

        $this->assertFalse($DB->record_exists(consent::TABLE, ['userid' => $user->id]));
        $this->assertSame(0, $DB->count_records(
            \block_eledia_aitutor\local\usage::TABLE,
            ['userid' => $user->id]
        ));
        // Other users' records are untouched.
        $this->assertTrue(consent::has_consented((int) $other->id));
        $this->assertSame(1, \block_eledia_aitutor\local\usage::count_today((int) $other->id));
    }

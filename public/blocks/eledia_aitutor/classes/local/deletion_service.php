 ...
 *  - LOCAL data (conversation pointers in {block_eledia_aitutor_conv}) is always
    ...
    public static function delete_all_for_user(int $userid, context $context, ?rag_client $client = null): array {
        ...
        if ($externalsupported && ($deleteusertool !== '' || !empty($records))) {
            try {
                ...
                debugging(
                    'block_eledia_aitutor: external deletion failed: ' . $e->getMessage(),
                    DEBUG_DEVELOPER
                );
            }
        }
        ...
        \block_eledia_aitutor\event\data_deletion_requested::create([
            'context' => $context,
            'userid' => $userid,

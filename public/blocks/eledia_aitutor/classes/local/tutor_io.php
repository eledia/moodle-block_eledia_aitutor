...
class tutor_io {
    ...
    public static function export(
        ...
        if (!$packer->archive_to_pathname($files, $path)) {
            throw new moodle_exception('error_export_failed', 'block_eledia_aitutor');
        }
        return $path;
    }
    ...
    private static function default_logo(): ?array {
        ...
        foreach (['logo.png', 'logo.svg', 'logo.jpg'] as $name) {
            $path = $CFG->dirroot . '/blocks/eledia_aitutor/pix/' . $name;
            if (is_readable($path)) {
    ...
    private static function is_safe_image(string $path): bool {
        ...
        // defence in depth (see block_eledia_aitutor_pluginfile()).

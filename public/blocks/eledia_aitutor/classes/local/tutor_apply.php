...
class tutor_apply {
    ...
    public static function to_site(array $settings, ?stored_file $logo, ?stored_file $avatar): void {
        ...
        foreach (registry::all() as $key => $entry) {
            ...
            if (array_key_exists($key, $settings) && $settings[$key] !== '' && $settings[$key] !== null) {
                set_config($cfgkey, $settings[$key], 'block_eledia_aitutor');
            } else {
                unset_config($cfgkey, 'block_eledia_aitutor');
            }
        }
    ...
    public static function instance_records(): array {
        ...
        return $DB->get_records('block_instances', ['blockname' => 'eledia_aitutor'], 'id ASC');
    }
    ...
    public static function to_instance_from_bundle(int $blockinstanceid, array $bundle): void {
        ...
        foreach ($images as $key => [$filearea, $image]) {
            ...
            $fs = get_file_storage();
            $fs->delete_area_files($blockcontext->id, 'block_eledia_aitutor', $filearea, 0);
    ...
    private static function replace_image(
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'block_eledia_aitutor', $filearea, $itemid);
        if ($source === null) {
            return;
        }
    ...
    private static function sync_storedfile_config(string $configname, ?stored_file $file): void {
        if ($file === null) {
            unset_config($configname, 'block_eledia_aitutor');
        } else {
            set_config($configname, '/' . $file->get_filename(), 'block_eledia_aitutor');
        }
    }
}

...
class ltm {
    ...
    public const PREF = 'block_eledia_aitutor_ltm_enabled';
    ...
    public static function sync_to_rag(int $userid, \context $context, ?rag_client $client = null): bool {
        ...
        try {
            ...
            $client->set_memory_optin($CFG->wwwroot, $token, self::is_enabled($userid), $toolname);
            return true;
            ...
            debugging(
                'block_eledia_aitutor: memory opt-in sync failed: ' . $e->getMessage(),
                DEBUG_DEVELOPER
            );
            return false;
        }

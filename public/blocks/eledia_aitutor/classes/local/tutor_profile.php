...
/**
 * CRUD for saved, site-wide tutor profiles (the block_eledia_aitutor_tutor table).
 *
...
class tutor_profile {
    ...
    private const TABLE = 'block_eledia_aitutor_tutor';
    ...
    public static function delete(int $id): void {
        ...
        foreach ([branding::TUTOR_LOGO_FILEAREA, branding::TUTOR_AVATAR_FILEAREA] as $area) {
            $fs->delete_area_files($syscontext->id, 'block_eledia_aitutor', $area, $id);
        }
    ...
    public static function store_image(int $id, string $filearea, string $filename, string $content): void {
        $fs = get_file_storage();
        $syscontext = \core\context\system::instance();
        $fs->delete_area_files($syscontext->id, 'block_eledia_aitutor', $filearea, $id);
    ...
    private static function trim_name(string $name): string {
        $name = trim($name);
        if ($name === '') {
            $name = get_string('tutor_untitled', 'block_eledia_aitutor');
        }

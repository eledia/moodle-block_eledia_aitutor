...
namespace block_eledia_aitutor\form;
...
class tutor_edit_form extends moodleform {
    ...
    protected function definition(): void {
        ...
        $mform->addElement(
            'text',
            'name',
            get_string('tutor_name', 'block_eledia_aitutor'),
            ['size' => 48]
        );
        ...
        $mform->addElement(
            'text',
            'shortname',
            get_string('tutor_shortname', 'block_eledia_aitutor'),
            ['size' => 32]
        );
        $mform->setType('shortname', PARAM_ALPHANUMEXT);
        $mform->addHelpButton('shortname', 'tutor_shortname', 'block_eledia_aitutor');
        if (!$isnew) {
        ...
        $mform->addElement(
            'textarea',
            'description',
            get_string('tutor_description', 'block_eledia_aitutor'),
            ['rows' => 2, 'cols' => 60]
        );
        ...
        $mform->addElement(
            'filemanager',
            'logo',
            get_string('reg_logo', 'block_eledia_aitutor'),
            null,
            $imageopts
        );
        $mform->addElement(
            'filemanager',
            'avatar',
            get_string('reg_avatar', 'block_eledia_aitutor'),
            null,
            $imageopts
        );
    ...
    private function add_registry_field(string $key, array $entry): void {
        $mform = $this->_form;
        $name = 'cfg_' . $key;
        $label = get_string_manager()->string_exists('reg_' . $key, 'block_eledia_aitutor')
            ? get_string('reg_' . $key, 'block_eledia_aitutor')
        ...
        switch ($entry['type']) {
            ...
            case 'textarea':
                ...
                break;
            case 'select':
                $options = ['' => get_string('tutor_notset', 'block_eledia_aitutor')];
            ...
            case 'checkbox':
                $mform->addElement('select', $name, $label, [
                    '' => get_string('tutor_notset', 'block_eledia_aitutor'),
                    '1' => get_string('yes'),
                    '0' => get_string('no'),
                ]);
                break;

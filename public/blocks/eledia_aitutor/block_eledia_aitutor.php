 ...
 */
class block_eledia_aitutor extends block_base {
    /**
    ...
    public function init(): void {
        $this->title = get_string('pluginname', 'block_eledia_aitutor');
    }
    ...
    public function instance_config_save($data, $nolongerused = false): void {
        if ($this->context) {
            foreach (
                [
                'logo' => \block_eledia_aitutor\local\branding::INSTANCE_LOGO_FILEAREA,
                'avatar' => \block_eledia_aitutor\local\branding::INSTANCE_AVATAR_FILEAREA,
                ] as $field => $filearea
            ) {
                if (!empty($data->$field)) {
    ...
    public function html_attributes(): array {
        ...
        $attributes['class'] .= ' block_eledia_aitutor';
        return $attributes;
    }
}

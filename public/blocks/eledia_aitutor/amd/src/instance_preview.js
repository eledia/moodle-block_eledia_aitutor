 ...
 *
 * The tutor's appearance is driven entirely by `--eat-*` CSS custom properties on the
 ...
 *
 * @module     block_eledia_aitutor/instance_preview
...
define([], function() {
     ...
     *
     * @param {Object} config tokenMap (key -> --eat-token), fieldTypes (key -> type),
    ...
    const init = function(config) {
        ...
        const panel = root.querySelector('.eledia_aitutor-panel');
        const targets = panel ? [root, panel] : [root];

        // Which registry key drives --eat-accent-dark, so we can tell whether the
        ...
        Object.keys(tokenMap).forEach(function(key) {
            if (tokenMap[key] === '--eat-accent-dark') {
                accentDarkKey = key;
            }
        });
        ...
        const applyToken = function(key) {
            ...
            // the operator has not set --eat-accent-dark explicitly.
            if (token === '--eat-accent') {
                ...
                if (!darkSet) {
                    setVar('--eat-accent-dark', (raw !== '' && isColour(raw)) ? darken(raw, 0.12) : null);
                }
            }
        };
        ...
        const applyFooter = function() {
            const node = root.querySelector('.eledia_aitutor-footer');
            if (!node) {
                return;
            }

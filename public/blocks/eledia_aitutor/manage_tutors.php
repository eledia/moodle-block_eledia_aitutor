...
use block_eledia_aitutor\form\tutor_edit_form;
...
$baseurl = new moodle_url('/blocks/eledia_aitutor/manage_tutors.php');
...
function block_eledia_aitutor_source_menu(): array {
    ...
    foreach (presets::menu() as $id => $label) {
        $menu['preset:' . $id] = get_string('tutor_preset', 'block_eledia_aitutor') . ': ' . $label;
    }
...
if ($action === 'export') {
    ...
    if ($data === null) {
        throw new moodle_exception('error_import_invalid', 'block_eledia_aitutor');
    }
    [$type, $id] = array_pad(explode(':', $source, 2), 2, '');
    $name = $type === 'preset'
        ? get_string(presets::all()[$id]['name'], 'block_eledia_aitutor')
...
if ($action === 'save') {
    ...
    if ($data = $form->get_data()) {
        ...
        redirect($baseurl, get_string('tutor_saved', 'block_eledia_aitutor'));
    }
...
if ($action === 'delete') {
    ...
    if (optional_param('confirm', 0, PARAM_BOOL)) {
        ...
        redirect($baseurl, get_string('tutor_deleted', 'block_eledia_aitutor'));
    }
    $profile = tutor_profile::get($id);
    echo $OUTPUT->header();
    shell::open(shell::ACTIVE_TUTORS);
    echo $OUTPUT->confirm(
        get_string('tutor_delete_confirm', 'block_eledia_aitutor', format_string($profile->name ?? '')),
...
if ($action === 'duplicate') {
    ...
    if ($data !== null) {
        ...
        $name = ($type === 'preset'
            ? get_string(presets::all()[$id]['name'], 'block_eledia_aitutor')
            : (string) (tutor_profile::get((int) $id)->name ?? 'tutor'))
            . ' ' . get_string('tutor_copysuffix', 'block_eledia_aitutor');
...
if ($action === 'applysite') {
    ...
    if (optional_param('confirm', 0, PARAM_BOOL)) {
        ...
        redirect($baseurl, get_string('tutor_applied_site', 'block_eledia_aitutor'));
    }
    echo $OUTPUT->header();
    shell::open(shell::ACTIVE_TUTORS);
    echo $OUTPUT->confirm(
        get_string('tutor_applysite_confirm', 'block_eledia_aitutor'),
...
if ($action === 'applyinstance') {
    ...
    if ($data !== null) {
        ...
        redirect($baseurl, get_string('tutor_applied_instance', 'block_eledia_aitutor'));
    }
    redirect($baseurl);
}
...
if ($action === 'importdo') {
    ...
    if ($data = $form->get_data()) {
        ...
        block_eledia_aitutor_stage_images($newid, $bundle);
        redirect(
            new moodle_url($baseurl, ['action' => 'edit', 'id' => $newid]),
            get_string('tutor_imported', 'block_eledia_aitutor')
        );
    }
 ...
 * @return void
 */
function block_eledia_aitutor_stage_images(int $profileid, array $bundle): void {
    foreach (['logo' => branding::TUTOR_LOGO_FILEAREA, 'avatar' => branding::TUTOR_AVATAR_FILEAREA] as $role => $area) {
...
if (!shell::is_available()) {
    echo $OUTPUT->heading(get_string('managetutors', 'block_eledia_aitutor'));
}
...
if ($action === 'new' || $action === 'edit') {
    ...
    echo html_writer::tag(
        'h3',
        $profile
            ? get_string('tutor_edit_title', 'block_eledia_aitutor', format_string($profile->name))
            : get_string('tutor_create_title', 'block_eledia_aitutor'),
...
echo html_writer::div(
    html_writer::tag('h3', get_string('nav_tutors', 'block_eledia_aitutor'), ['class' => 'eat-section-title']) .
    html_writer::div(
        $libraryaction(
...
foreach (tutor_profile::get_all() as $profile) {
    echo block_eledia_aitutor_tutor_card(
        $baseurl,
        'profile:' . $profile->id,
        $profile->name,
        get_string('tutor_custom', 'block_eledia_aitutor'),
        'custom',
...
echo html_writer::tag(
    'h3',
    get_string('tutor_templates', 'block_eledia_aitutor'),
...
foreach (presets::menu() as $pid => $plabel) {
    echo block_eledia_aitutor_tutor_card(
        ...
        'preset:' . $pid,
        $plabel,
        get_string('tutor_preset', 'block_eledia_aitutor'),
        'preset',
...
if ($instances) {
    $sources = block_eledia_aitutor_source_menu();
    ...
    echo html_writer::tag(
        'h3',
        get_string('tutor_instances', 'block_eledia_aitutor'),
        ['class' => 'eat-section-title']
    );
    ...
    foreach ($instances as $bi) {
        ...
        $applyform = html_writer::tag(
            ...
            html_writer::tag(
                ...
                [
                    ...
                    'aria-label' => get_string('tutor_apply', 'block_eledia_aitutor'),
                    'title' => get_string('tutor_apply', 'block_eledia_aitutor'),
                ]
            ),
        ...
        echo html_writer::div(
            ...
            block_eledia_aitutor_preview($src['settings'], get_string('pluginname', 'block_eledia_aitutor')) .
            $applyform .
 ...
 */
function block_eledia_aitutor_pcol(array $settings, string $key, string $default): string {
 ...
 * @return string HTML.
 */
function block_eledia_aitutor_preview(array $settings, string $name): string {
    $accent = block_eledia_aitutor_pcol($settings, 'brandaccent', '#1e3f59');
    $accentfg = block_eledia_aitutor_pcol($settings, 'tok_accentcontrast', '#ffffff');
    $body = block_eledia_aitutor_pcol($settings, 'brandsurface', '#f4f6f8');
    $botbg = block_eledia_aitutor_pcol($settings, 'brandbotbubble', '#ffffff');
    $botfg = block_eledia_aitutor_pcol($settings, 'tok_botfg', '#1e3f59');
    $userbg = block_eledia_aitutor_pcol($settings, 'brandbubble', '#fce9db');
    $userfg = block_eledia_aitutor_pcol($settings, 'tok_userfg', '#1e3f59');
 ...
 * @return string HTML.
 */
function block_eledia_aitutor_tutor_card(
    moodle_url $baseurl,
    string $source,
    string $name,
    string $typelabel,
    ...
    $actions = html_writer::link(
        ...
        [
            ...
            'aria-label' => get_string('tutor_applysite', 'block_eledia_aitutor'),
            'title' => get_string('tutor_applysite', 'block_eledia_aitutor'),
        ]
    );
    ...
    return html_writer::div(
        block_eledia_aitutor_preview($settings, $persona !== '' ? $persona : $name) . $body,
        'eat-tutor-card'
    );
}

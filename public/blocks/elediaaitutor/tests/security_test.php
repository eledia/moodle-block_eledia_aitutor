 ...
 * @covers      \block_elediaaitutor\local\security
...
final class security_test extends \advanced_testcase {
    ...
    public function test_validated_rag_url_missing_throws(): void {
        $this->resetAfterTest();
        set_config('ragserverurl', '', 'block_elediaaitutor');
    ...
    public function test_validated_rag_url_rejects_http_by_default(): void {
        $this->resetAfterTest();
        set_config('ragserverurl', 'http://rag.example.com/mcp', 'block_elediaaitutor');
        $this->expectException(\moodle_exception::class);
        security::validated_rag_url();
    }
    ...
    public function test_validated_rag_url_allows_http_when_opted_in(): void {
        $this->resetAfterTest();
        set_config('ragserverurl', 'http://localhost:8080/mcp', 'block_elediaaitutor');
        set_config('allowinsecuretransport', 1, 'block_elediaaitutor');
        $url = security::validated_rag_url();
    ...
    public function test_validated_rag_url_rejects_other_schemes(): void {
        $this->resetAfterTest();
        set_config('ragserverurl', 'file:///etc/passwd', 'block_elediaaitutor');
        set_config('allowinsecuretransport', 1, 'block_elediaaitutor');
        $this->expectException(\moodle_exception::class);
        security::validated_rag_url();
    }
    ...
    public function test_validate_message_enforces_length(): void {
        $this->resetAfterTest();
        set_config('maxmessagelength', 10, 'block_elediaaitutor');
        $this->expectException(\moodle_exception::class);
    ...
    public function test_rate_limit_trips(): void {
        $this->resetAfterTest();
        set_config('ratelimitperminute', 3, 'block_elediaaitutor');
        for ($i = 0; $i < 3; $i++) {
    ...
    public function test_rate_limit_disabled(): void {
        $this->resetAfterTest();
        set_config('ratelimitperminute', 0, 'block_elediaaitutor');
        for ($i = 0; $i < 50; $i++) {

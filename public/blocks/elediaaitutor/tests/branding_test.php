...
final class branding_test extends \advanced_testcase {
    ...
    public function test_preset_palette(): void {
        ...
        foreach (presets::settings('hal') as $key => $value) {
            set_config(registry::sitekey($key), $value, 'block_elediaaitutor');
        }
    ...
    public function test_persona_assembly(): void {
        $this->resetAfterTest();
        set_config('persona', 'Site Tutor', 'block_elediaaitutor');
        set_config('persona_tone', 'friendly', 'block_elediaaitutor');

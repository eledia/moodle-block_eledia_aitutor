...
class TutorChat {
     ...
     */
    conversationKey() {
        return 'elediaaitutor_conv_' + this.config.contextid + '_' + (this.config.courseid || 0);
    }
    ...
    restoreStyle() {
        ...
        let stored = null;
        try {
            stored = window.sessionStorage.getItem('elediaaitutor_style_' + this.config.contextid);
        } catch (e) {
            return;
        }
        ...
        this.panel.querySelectorAll('[data-action="style"]').forEach((chip) => {
            ...
            chip.classList.toggle('elediaaitutor-stylechip-active', active);
    ...
    setStyle(el) {
        ...
        try {
            window.sessionStorage.setItem('elediaaitutor_style_' + this.config.contextid, style);
        } catch (e) {
    ...
    applyBrand(modal) {
        ...
        root.classList.add('elediaaitutor-modal');
    ...
    saveLtm(toggle, root) {
        Ajax.call([{
            methodname: 'block_elediaaitutor_set_ltm',
    ...
    performDeleteAll(privacyroot) {
        Ajax.call([{
            methodname: 'block_elediaaitutor_delete_my_data',
            args: {contextid: this.config.contextid}
            ...
            if (status) {
                const done = await getString('deleteall_done', 'block_elediaaitutor', response.localdeleted);
...
export default {
    ...
    init: function(uniqid) {
        ...
        const requests = STRING_DEFS.map(([, key, component]) => ({
            key: key,
            component: component || 'block_elediaaitutor'
        }));

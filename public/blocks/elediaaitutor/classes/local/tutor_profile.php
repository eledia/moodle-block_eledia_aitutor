...
/**
 * CRUD for saved, site-wide tutor profiles (the block_elediaaitutor_tutor table).
 *
...
class tutor_profile {
    ...
    private const TABLE = 'block_elediaaitutor_tutor';
    ...
    private static function trim_name(string $name): string {
        $name = trim($name);
        if ($name === '') {
            $name = get_string('tutor_untitled', 'block_elediaaitutor');
        }

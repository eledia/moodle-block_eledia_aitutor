 ...
 *
 * Every provisioned token is attributed to this component ('block_elediaaitutor')
...
class token_provider {
    ...
    public const COMPONENT = 'block_elediaaitutor';
    ...
    public static function require_available(): void {
        if (!self::is_connector_available()) {
            throw new moodle_exception('error_connector_missing', 'block_elediaaitutor');
        }
        $serviceid = security::mcp_service_id();
        if ($serviceid <= 0) {
            throw new moodle_exception('error_service_not_configured', 'block_elediaaitutor');
        }
        $services = call_user_func([self::MCP_API, 'get_services']);
        if (!isset($services[$serviceid])) {
            throw new moodle_exception('error_service_unavailable', 'block_elediaaitutor');
        }
    }
    ...
    public static function get_token(int $userid): string {
        ...
        $cache = cache::make('block_elediaaitutor', 'usertoken');
    ...
    private static function mint_token(int $userid, int $serviceid, cache $cache, string $cachekey): string {
        ...
        try {
            ...
            debugging('block_elediaaitutor: revoke of stale tokens failed: ' . $e->getMessage(), DEBUG_DEVELOPER);
        }

        $lifetime = security::token_lifetime();
        $validuntil = $lifetime > 0 ? time() + $lifetime : 0;
        $label = get_string('tokenlabel', 'block_elediaaitutor');
        ...
        if ($token === '') {
            throw new moodle_exception('error_token_provision_failed', 'block_elediaaitutor');
        }

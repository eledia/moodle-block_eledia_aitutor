...
class security {
    ...
    public const CONFIG_COMPONENT = 'block_elediaaitutor';
    ...
    public static function validated_rag_url(): moodle_url {
        ...
        if ($raw === '') {
            throw new moodle_exception('error_rag_url_missing', 'block_elediaaitutor');
        }

        $parts = parse_url($raw);
        if ($parts === false || empty($parts['scheme']) || empty($parts['host'])) {
            throw new moodle_exception('error_rag_url_invalid', 'block_elediaaitutor');
        }
    ...
    public static function enforce_rate_limit(int $userid): void {
        ...
        $cache = cache::make('block_elediaaitutor', 'ratelimit');

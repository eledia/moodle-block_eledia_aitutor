...
namespace block_elediaaitutor\form;
...
class tutor_edit_form extends moodleform {
    ...
    private function add_registry_field(string $key, array $entry): void {
        $mform = $this->_form;
        $name = 'cfg_' . $key;
        $label = get_string_manager()->string_exists('reg_' . $key, 'block_elediaaitutor')
        ...
        switch ($entry['type']) {
            ...
            case 'textarea':
                ...
                break;
            case 'select':
                $options = ['' => get_string('tutor_notset', 'block_elediaaitutor')];

...
namespace block_elediaaitutor\task;
...
class prune_question_log extends scheduled_task {
    ...
    public function get_name(): string {
        return get_string('task_prune_question_log', 'block_elediaaitutor');
    }
    ...
    public function execute(): void {
        ...
        if ($days <= 0) {
            mtrace('block_elediaaitutor: question-log pruning is disabled (retention = 0).');
            return;
        }

        $deleted = question_log::prune($days);
        mtrace("block_elediaaitutor: pruned {$deleted} logged question(s) older than {$days} day(s).");
    }
}

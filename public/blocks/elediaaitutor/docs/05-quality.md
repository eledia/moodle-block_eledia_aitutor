...
`block_elediaaitutor.php`, `lib.php`, `edit_form.php`,
...
wird spaeter ueber `block_elediaaitutor_pluginfile()` (`lib.php:113`) mit
...
**Trust-Boundary**
Upload erfordert `block/elediaaitutor:manage` bzw. `moodle/site:config`. Es ist
...
1. Auslieferungsschicht: `block_elediaaitutor_pluginfile()` (`lib.php`) liefert
...
**Beschreibung**
`.elediaaitutor-history-open` (`templates/conversation_list.mustache:38`) ist der
...
**Erwartet**
`.elediaaitutor-history-open:focus-visible` erhaelt einen sichtbaren Ring analog
zu den uebrigen interaktiven Controls.

**Fix 2026-06-27**
`:focus-visible` fuer `.elediaaitutor-history-open` ergaenzt.
...
**Fix 2026-06-27**
Privacy-Abschnittsueberschriften nutzen nun `h3.elediaaitutor-privacy-heading`.
